...
def gridFill(xmax, ymax):
	...
	for k in range(xmax):
		for l in range(ymax):
			if(k%xspacing==0 and l%yspacing == 0):
				T[k,l] = tempHigh
			elif(k== 0 or l ==0 or l ==ymax or k == xmax):
				T[k,l] = tempLow
			else:
				T[k,l] = tempLow
	return T
...
# X Derivative
plt.subplot(131)
plt.pcolor(X, Y, functx(dT_x),  cmap='jet',  vmin = np.min(dT_x), vmax = np.max(dT_x))
plt.title(r"$\displaystyle\partial_xT(x,y)$", fontsize = 12, color ='black')
plt.colorbar()
# Y Derivative
plt.subplot(132)
plt.pcolor(X, Y, functy(dT_y),  cmap='jet',  vmin = np.min(dT_y) , vmax = np.max(dT_y))
plt.title( r"$\displaystyle\partial_yT(x,y)$", fontsize = 12, color ='black')
plt.colorbar()
# Surface Temperature
plt.subplot(133)
plt.pcolor(X, Y, functz(T),  cmap='jet',  vmin = tempLow , vmax = tempHigh)
